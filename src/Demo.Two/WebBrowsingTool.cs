...
internal sealed partial class WebBrowsingTool : AIFunction
{
    ...
    private async Task<string?> CheckAccessAsync(Uri uri, CancellationToken cancellationToken)
    {
        ...
        if (_options.AllowedHosts is { Count: > 0 } allowedHosts)
        {
            if (allowedHosts.Any(pattern => HostMatchesPattern(host, pattern)))
            {
    ...
    private static partial class HtmlToMarkdownConverter
    {
        public static string Convert(string html)
        {
            ...
            Console.WriteLine("Markdown page from web");
            Console.WriteLine(content.Trim());
            Console.WriteLine("End of Markdown page  from web");
